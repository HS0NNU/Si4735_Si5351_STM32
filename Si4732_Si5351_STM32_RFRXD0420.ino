...
void set_frequency(short dir) {
  ...
  if (MenueEnable == 0)
  {
    ...
    if (freq >= 438000000) freq = 438000000; // if (freq >= 230000000) freq = 230000000
    if (dir == -1) freq = freq - fstep;
...
void tunegen() {
  if (MenueEnable == 0)
  {
    ...
    {
      si5351.set_freq(((freq - (interfreq * 1000ULL)) / 16) * 100ULL, SI5351_CLK0); //si5351.set_freq((freq + (interfreq * 1000ULL)) * 100ULL, SI5351_CLK0)
    }
  }
}
...
void stepCheck() {
  ...
  if (digitalRead(button2) == LOW && (fstep == 1)) {		// some menu stuck at 1 Hz.-step escape to 100 Hz.
   fstep = 100;
    steps = "100Hz";
    delay (300);
  }
